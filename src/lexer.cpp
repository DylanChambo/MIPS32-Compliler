...
Lexer::Lexer(const string &code) { m_code = code; }
...
vector<Token> Lexer::tokenize() {
  ...
  while (peek() != '\0') {
    // If the first character is a alpha character, it is either a keyword or an
    // identifier
    if (isalpha(peek())) {
      currentToken.push_back(consume());
      // Keep adding characters till we reach a non-alphanumeric or '_'
      // character
      ...
      if (currentToken == "int")
        ...
        tokens.push_back(
            {.type = TokenType::IDENTIFIER, .value = currentToken});
    ...
    // if current char is whitespace consume it
      ...
      cerr << "Unexpected char \'" << peek() << "\' at line " << m_line
           << ", position " << m_char << "!" << endl;
      exit(EXIT_FAILURE);
    }
  }
